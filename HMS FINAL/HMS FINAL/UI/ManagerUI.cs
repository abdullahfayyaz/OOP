...
namespace HMS_FINAL.UI
{
    class ManagerUI
    {
        ...
        public static void managerInterface()
        {
            ...
            while (true)
            {
                ...
                else if (option_manager == "6")
                {
                    ...
                    Room info = new Room();
                    int rooms = PersonDL.availableRooms(info.TotalRoom);
                    Console.ForegroundColor = ConsoleColor.DarkYellow;
                    Console.WriteLine("Total Rooms: " + info.TotalRoom);
        ...
        {
            int index = CustomerUI.customerIndex();
            if (index == -1)
            {
                Console.ForegroundColor = ConsoleColor.DarkRed;
            ...
            else if (index != -2)
            {
                while (true)
                {
                    ...
                    else if (option_update == "4")
                    {
                        ...
                        if (valid_stay == true)
                        {
                            PersonDL.updateStayDay(no_of_stay, index);
                        }
                        else if (valid_stay == false)
                        {
                            Interface.wrongInput();
        ...
        public static void searchCustomerInterface()
        {
            ...
            {
                PersonDL.searchCustomerInList(index);
            }
        }
        ...
        // Checkout
        public static void checkoutInterface()
        {
            int index = CustomerUI.customerIndex();
            if (index == -1)
            {
            ...
            else if (index != -2)
            {
                Interface.printHeader();
                Interface.subMenu("Checkout");
                PersonDL.checkoutBill(index);
                PersonDL.CheckOutFromList(index);
                PersonDL.removePerson(index);
                PersonDL.saveData();
                Console.ForegroundColor = ConsoleColor.Cyan;
