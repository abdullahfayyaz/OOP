...
namespace HMS_FINALIZED
{
    public partial class ReviewUC : UserControl
    {
        ...
        private void ClearCustomerData()
        {
            ...
            TextReview.Clear();
            TextReview.BorderColor = Color.FromArgb(213, 218, 223);
        }
        ...
        private void ReviewGradientButton_Click(object sender, EventArgs e)
        {
            ...
            NameField(TextName);
            CNIC_Field(TextID);
            if (Validation.isValid(TextName.Text) && Validation.id_check(TextID.Text) && TextReview.Text != "")
            {
                ...
                {
                    PersonDL.customerReview(TextReview.Text, index);
                    PersonDL.saveData();
                    ClearCustomerData();
                    MessageBox.Show("Thank you for your review", "Review", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            else
            {
                MessageBox.Show("Review can't be empty", "Review", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
    }
}
