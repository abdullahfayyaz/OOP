        ...
        void loadGrid(string fileName)
        {
            ...
            for (int row = 0; row < this.rows; row++)
            {
                ...
                for (int col = 0; col < this.cols; col++)
                {
                    ...
                    {
                        Obstacles obstacle = new Obstacles(displayIamge, cell, GameObjectType.OBSTACLE);
                        Game.obstacle.Add(obstacle);
                    }
                    if (type == GameObjectType.OBSTACLE2)
                    {
                        Obstacles obstacle = new Obstacles(displayIamge, cell, GameObjectType.OBSTACLE2);
                        Game.obstacle.Add(obstacle);
                    }
                }
            }
